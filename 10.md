...
- however usually this is not done because structures are
  ...
  - for better :eyes: code readability, members are sometimes prefixed with a
...
```
   - another reason is when looking for variable names in a big
	  source :eyes: code repository (using tags or cstyle), there would be
	  large amount of generally named variables like "port", "size",
	  etc in various source :eyes: code files.  However, with the prefix,
...
Note: for compilation it is only necessary to compile the *.c files
   ...
   Technically, animals.h contains :eyes: code, however, given it is included
   in a .c file it is not necessary to compile it individually.

------------------------------------------------------------------------------

:wrench: Task: use the :eyes: code from previous task and implement (in separate .c file)
...
:wrench: task: write a macro (or start with a function with hard:eyes: coded values) that will
...
See the following :eyes: code on how to use getopt(3):
