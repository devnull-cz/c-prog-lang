...
whose value is the address of the 1st element of the array, ie. the 1st char.
`{ 'a', 'b', ... }` is an initializer for a static array.  So, the following
will not compile:
...
In the first case, the string is only used to initialize the array.  You cannot
assign anything to "a" after that but you can change its array elements.

In the second case, a static string is created and stored in the binary (ie. the
program file), and "p" contains its address, ie. an address of the first
character.  One can change the "p" variable to point to something else but it's
...
Wikipedia: In computing, a bus error is a fault raised by hardware, notifying an
operating system (OS) that a process is trying to access memory that the CPU
cannot physically address: an invalid address for the address bus, hence the
name.
