...
- For example, the `1`, `7`, and `20000` integer constants are always integers
  of type `int` **if they fit** in.
...
- `float` numbers are automatically converted to `double` typed numbers if used
  as arguments in functions with variable number of arguments (known as
  *variadic function*), i.e. like printf()
