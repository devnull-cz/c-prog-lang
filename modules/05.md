...
We will be citing the standard in the text below.  The C standards are not free
but the drafts are.  The latest draft can be used as the standard since the only
change from the draft is that the latest draft is labeled as the standard.  The
latest C99 draft: http://www.open-std.org/JTC1/sc22/wg14/www/docs/n1256.pdf
...
Integer conversion:	converting integers (eg. assigning a `signed long` to an
			`unsigned char`, or assigning an `unsigned int` to a
			`signed char`)
