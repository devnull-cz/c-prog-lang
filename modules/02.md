...
- "char" and "short int" is converted to "int" in variadic functions (we will
  ...
  OK as the compiler will first convert "c" to "int", then put it on the stack
  (x32 passing argument convention) or in a register (x64 convention).
```C
	/* OK */
	char c;
	printf("%d\n", c);
...
- if you use a larger number than fits within a byte as an argument for the %c
  conversion, the higher bits are trimmed.  The rule is that the `int` argument
  is converted to `unsigned char`.  More on integer conversion in upcoming
  lectures.
