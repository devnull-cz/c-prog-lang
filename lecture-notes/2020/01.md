...
- `printf()` can use *conversion specifications*, each starts with `%`
	...
	- number of conversions must match the number of arguments
		...
		- the modern compilers will warn you but it will let you do it (use
		  `-Werror` to treat warnings as errors). For older compilers there is 
		  usually a program called `lint` that will check for such problems.
